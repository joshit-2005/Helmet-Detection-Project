...
def load_head_helmet_data(images_dir, annotations_dir):
    images, labels = [], []
    ...
    for xml_file in xml_files:
        try:
            ...
            for obj in root.findall('object'):
                ...
                if xmax <= xmin + 32 or ymax <= ymin + 32: continue
...
def main():
    print("🏍️ TWO-STAGE HELMET DETECTOR TRAINING")
    print("Face/Head crops → Helmet classifier (95%+)")
    
    X, y = load_head_helmet_data("images", "annotations")
    print(f"✅ Loaded: {len(X)} head crops")
    print(f"   Helmet: {sum(y)} ({100*sum(y)/len(y):.1f}%) | No Helmet: {len(y)-sum(y)}")
    
    if len(X) == 0:
        print("❌ NO DATA! Check:")
        print("   - images/*.png exists")
        print("   - annotations/*.xml has 'With Helmet'/'Without Helmet'")
        print("   - Bounding boxes >32px")
        return
    ...
    callbacks = [
        ...
        keras.callbacks.ModelCheckpoint('head_helmet_model.h5', save_best_only=True)
    ]
    ...
    model.save('head_helmet_model.h5')
    print("💾 SAVED: head_helmet_model.h5")
    print("\n✅ READY for Flask app!")
